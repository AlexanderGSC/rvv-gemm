        ...
        for (sj = 0; sj < m; sj += TILE_SIZE) {
            for (sk = 0; sk < p; sk += TILE_SIZE) {
                ...
                for (j = sj; j < j_end; j += 2*vl) {
                    vl = __riscv_vsetvl_e32m2(j_end - j);

                    // 1. BUCLE PRINCIPAL CON UNROLL (De 4 en 4 filas)
                    // Nos detenemos 4 elementos antes del final real
                    size_t i_unroll_end = i_end >= 3 ? i_end - 3 : si;
                    for (i = si; i < i_unroll_end; i += 4) {
                        // Tenemos 4 acumuladores independientes para 4 filas distintas de C
                        vfloat32m4_t v_c0 = __riscv_vle32_v_f32m4(&c[i+0][j], vl);
                        vfloat32m4_t v_c1 = __riscv_vle32_v_f32m4(&c[i+1][j], vl);
                        vfloat32m4_t v_c2 = __riscv_vle32_v_f32m4(&c[i+2][j], vl);
                        vfloat32m4_t v_c3 = __riscv_vle32_v_f32m4(&c[i+3][j], vl);

                        for (k = sk; k < k_end; k++) {
                            // Una sola carga de B sirve para alimentar los 4 acumuladores
                            vfloat32m4_t v_b = __riscv_vle32_v_f32m4(&b[k][j], vl);
                            // Cargamos los 4 escalares correspondientes de A
                            v_c0 = __riscv_vfmacc_vf_f32m4(v_c0, a[i+0][k], v_b, vl);
                            v_c1 = __riscv_vfmacc_vf_f32m4(v_c1, a[i+1][k], v_b, vl);
                            v_c2 = __riscv_vfmacc_vf_f32m4(v_c2, a[i+2][k], v_b, vl);
                            v_c3 = __riscv_vfmacc_vf_f32m4(v_c3, a[i+3][k], v_b, vl);
                        }
                        __riscv_vse32_v_f32m4(&c[i+0][j], v_c0, vl);
                        __riscv_vse32_v_f32m4(&c[i+1][j], v_c1, vl);
                        __riscv_vse32_v_f32m4(&c[i+2][j], v_c2, vl);
                        __riscv_vse32_v_f32m4(&c[i+3][j], v_c3, vl);
                    }
                    // 2. BUCLE DE LIMPIEZA (Procesa el residuo de 1 en 1)
                    for (; i < i_end; i++) {
                        vfloat32m2_t v_c = __riscv_vle32_v_f32m2(&c[i][j], vl);
                        for (k = sk; k < k_end; k++) {
                            vfloat32m2_t v_b = __riscv_vle32_v_f32m2(&b[k][j], vl);
                            v_c = __riscv_vfmacc_vf_f32m2(v_c, a[i][k], v_b, vl);
                        }
                        __riscv_vse32_v_f32m2(&c[i][j], v_c, vl);
                    }
                }
            }
        }
