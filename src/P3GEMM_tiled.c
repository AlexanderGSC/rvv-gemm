...
void GEMM(size_t n, size_t p, size_t m, float **a, float **b, float **c) {
    ...
    // 1. Bucles externos: Dividen el problema en bloques
    for (si = 0; si < n; si += TILE_SIZE) {
        for (sj = 0; sj < m; sj += TILE_SIZE) {
            for (sk = 0; sk < p; sk += TILE_SIZE) {
                ...
                for (j = sj; j < j_end; j += vl) {
                    // Calculamos el vl para el ancho del bloque
                    vl = __riscv_vsetvl_e32m8(j_end - j);

                    for (i = si; i < i_end; i++) {
                        // ¡MAGIA! Cargamos el acumulador de C una sola vez para este bloque
                        vfloat32m8_t v_c = __riscv_vle32_v_f32m8(&c[i][j], vl);

                        for (k = sk; k < k_end; k++) {
                            float scalar_a = a[i][k];
                            vfloat32m8_t v_b = __riscv_vle32_v_f32m8(&b[k][j], vl);
                            v_c = __riscv_vfmacc_vf_f32m8(v_c, scalar_a, v_b, vl);
                        }

                        // Hacemos el STORE solo una vez al terminar el sub-bloque K
